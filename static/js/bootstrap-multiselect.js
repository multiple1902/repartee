...
!function ($) {
	...
	function Multiselect(select, options) {
		...
		this.container = $(this.options.container)
				.append('<button type="button" style="width:' + this.options.width + '" class="dropdown-toggle ' + this.options.button + '" data-toggle="dropdown">' + this.options.text($('option:selected', select)) + ' <b class="caret pull-right"></b></button>')
		...
		$('option', this.select).each($.proxy(function(index, element) {
			...
			$('ul', this.container).append('<li><a href="javascript:void(0);" style="padding:0;"><label style="margin:0;padding: 3px 10px 3px 5px;width:100%;height:100%;cursor:pointer;text-align:left;"><input style="margin-bottom:5px;" type="checkbox" value="' + $(element).val() + '" /> ' + $(element).text() + '</label</a></li>');
		...
		$('ul li input[type="checkbox"]', this.container).on('change', $.proxy(function(event) {
			...
			$('button', this.container).html(this.options.text(options) + ' <b class="caret pull-right"></b>');
	...
	Multiselect.prototype = {
		...
		refresh: function() {
			...
			$('button', this.container).html(this.options.text($('option:selected', this.select)) + ' <b class="caret pull-right"></b>');
		},
